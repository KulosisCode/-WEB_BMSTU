...
import { GUESTS_TABLE, build_connect, performDelete, performInsert, performQuery, performUpdate } from "./common";
import { validatePerson } from "./dbCheck/validPerson";
import { DTOPerson } from "../server/models";

export class DbGuestRepo implements IPersonRepo {
    conn: Pool;
    ...
    async addPerson(person: Person): Promise<boolean> {
        ...
        const query = `INSERT INTO ${GUESTS_TABLE} (id_login, name, age, email, address) VALUES \
    ...
    async getPerson(id: number): Promise<Person | null> {
        const query = `SELECT * FROM ${GUESTS_TABLE} WHERE id = ${id};`
    ...
    async getIdPersonByIdLogin(id_login: number): Promise<number | null> {
        const query = `SELECT id FROM ${GUESTS_TABLE} WHERE id_login = ${id_login};`
        const qres = await performQuery(query, this.conn);    
        return  qres && qres.rowCount 
                ? (qres.rows[0].id) 
                : null;
    ...
    async updatePerson(id: number, name: string, age: number, email: string, address: string) {
        const query = `UPDATE ${GUESTS_TABLE} SET name = '${name}', \
    ...
    async removePerson(id: number) {
        const query = `DELETE FROM ${GUESTS_TABLE} WHERE id = ${id};`;
        await performDelete(query, this.conn);
        return true;
    }

    async getPersons(): Promise<Person[] | null> {
        const query = `SELECT * FROM ${GUESTS_TABLE};`;
